browser_allsamp_ui <- function(id, all_exp, browser_options,
  ...
  columns_to_show <- c(
    "BioProject", "CONDITION", "INHIBITOR",
  ...
  tabPanel(
    ...
    fluidRow(
      column(1, div(
        ...
        actionButton(ns("toggle_settings"), "",
          icon = icon("sliders-h"),
          ...
          tabsetPanel(
            tabPanel(
              ...
              fluidRow(
                ...
                experiment_input_select(
                  experiments, ns, browser_options,
                  "default_experiment_meta"
                ),
              ),
              fluidRow(
                column(6, metadata_input_select(ns, metadata, TRUE)),
              ...
              fluidRow(
                column(6, region_view_select(ns, "region_type", "Select region to view")),
            ...
            tabPanel(
              ...
              fluidRow(
                ...
                numericInput(
                  ns("collapsed_introns_width"), "Collapse Introns (nt flanks)",
                  introns_width
                )
              ),
              ...
              fluidRow(
                column(6, heatmap_color_select(ns)),
                column(6, sliderInput(ns("color_mult"), "Color scale zoom",
                  min = 1, max = 10,
                  value = 3
                ))
              ),
              fluidRow(
                column(6, gene_input_select(ns, label = "Sort by other gene", id = "other_gene")),
              ...
              fluidRow(
                column(6, numericInput(ns("min_count"), "Minimum counts", min = 0, value = 100)),
                column(6, sliderInput(ns("kmer"), "K-mer length",
                  min = 1, max = 20,
      ...
      column(
        2, prettySwitch(ns("viewMode"), "Genomic View",
          value = viewMode,
          status = "success", fill = TRUE, bigger = TRUE
        ),
        prettySwitch(ns("other_tx"), "Full annotation",
          value = full_annotation,
          status = "success", fill = TRUE, bigger = TRUE
        ),
        prettySwitch(ns("collapsed_introns"), "Collapse introns",
          value = FALSE,
          status = "success", fill = TRUE, bigger = TRUE
        )
      ),
      column(2, motif_input_select(ns)),
      column(2, sliderInput(ns("clusters"), "K-means clusters",
        min = 1, max = 20,
        value = 5
      ))
    ),
    ...
    function(input, output, session, all_exp = all_experiments) {
      ...
      controller <- eventReactive(input$go,
        click_plot_browser_allsamp_controller(
          ...
          orderByMetadataField = input$metadata,
      ...
      plot_object <- reactive(get_meta_browser_plot(
        table()$table,
      ...
      output$c <- renderUI(
        ...
        bindCache(
          controller()$table_hash, input$heatmap_color,
          isolate(input$color_mult)
        ) %>%
        bindEvent(plot_object(), ignoreInit = FALSE, ignoreNULL = TRUE)
      ...
      meta_and_clusters <- reactive(
        allsamples_metadata_clustering(
          table()$metadata_field, plot_object(),
      ...
      output$result_table <- renderDT(
        cbind(
          attr(meta_and_clusters()$meta, "runIDs"),
          meta_and_clusters()$meta
        )[, c("index", "order") := NULL],
        extensions = "Buttons",
        filter = "top",
        options = list(
