browser_ui <- function(id, all_exp, browser_options, gene_names_init,
  ...
  tabPanel(
    ...
    fluidRow(
          ...
          tabsetPanel(
            tabPanel(
              ...
              fluidRow(
                ...
                column(1, actionButton(ns("select_all_btn"), "",
                  icon = icon("check"),
                  class = "btn btn-sm btn-primary", title = "Select all"
                ))
              ),
              fluidRow(prettySwitch(ns("unique_align"), "Unique alignments",
                value = FALSE,
            ...
            tabPanel(
              ...
              fluidRow(
                textInput(ns("genomic_region"), "Genomic region", ""),
      ...
      column(
        1, prettySwitch(ns("viewMode"), "Genomic View",
          value = viewMode,
          status = "success", fill = TRUE, bigger = TRUE
        ),
...
go_when_input_is_ready <- function(input, browser_options, fired, kickoff, libs) {
  ...
  if (!nzchar(input$gene) || !nzchar(input$tx)) {
    return()
  }
  if (!identical(input$gene, browser_options[["default_gene"]])) {
    return()
  }
  if (!identical(input$tx, browser_options[["default_isoform"]])) {
    return()
  }
